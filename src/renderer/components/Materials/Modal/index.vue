    ...
    <div :id="name" :class="`Modal size-${size}`">
      ...
      <div class="ModalActions">
        <slot name="footer" :accept="accept" :refuse="refuse" :close="hide">
          <button type="button" class="btn btn-refuse" v-if="cancelOnly || okCancel" @click="refuse">
            {{ cancelLabel }}
          </button>
          <button type="button" class="btn btn-accept" v-if="okOnly || okCancel" @click="accept">
            {{ okLabel }}
          </button>
        </slot>
      </div>
      <slot name="CloseButton" :close="hide">
        <button type="button" class="ModalCloseBtn" @click="hide"></button>
      </slot>
    </div>
  </div>
</template>
...
export default {
  ...
  mounted() {
    ModalStore.add(this);
    this.oldFocusElement = document.activeElement;
  },
  destroyed() {
